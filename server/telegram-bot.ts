...
سأقوم بتحليل طلبك وتنفيذه مباشرة في النظام.`);
  ...
  bot.on('text', async (ctx) => {
    ...
    try {
      ...
      const processingMessage = await ctx.reply(`🔄 جاري تحليل ومعالجة طلبك: "${text}"`);
      
      // تحديد ما إذا كان الطلب يتعلق بتحليل النظام
      if (text.includes('درس النظام') || text.includes('تحليل النظام') || 
          text.includes('فحص الكود') || text.includes('تقييم النظام')) {
        await ctx.reply('🔍 جاري تحليل النظام بالكامل وتقديم اقتراحات...');
        
        // تحليل النظام بالكامل
        const analysisResult = await executeCommand('تحليل النظام بالكامل');
        await ctx.reply(analysisResult);
        
        // اقتراح تحسينات
        await ctx.reply('💡 جاري تقديم اقتراحات لتحسين النظام...');
        const suggestionsResult = await executeCommand('اقتراح تحسينات');
        await ctx.reply(suggestionsResult);
        
        await ctx.reply('هل ترغب في تنفيذ أي من هذه التحسينات؟ يمكنك الرد بـ "نفذ التحسين رقم X" أو وصف التحسين الذي ترغب في تنفيذه.');
        return;
      }
      
      // تحديد ما إذا كان الطلب يتعلق بتنفيذ تحسين أو اقتراح
      if (text.includes('نفذ') && (text.includes('التحسين') || text.includes('الاقتراح'))) {
        await ctx.reply('🚀 جاري تنفيذ التحسين المطلوب بشكل تلقائي...');
        const implementResult = await executeCommand(`نفذ تلقائيًا: ${text}`);
        await ctx.reply(implementResult);
        return;
      }
      
      // معالجة الطلب كأمر عام للتنفيذ المباشر
      const result = await executeCommand(text);
      
      // تحديث رسالة المعالجة بالنتيجة
      ...
      // إرسال النتيجة التفصيلية
      await ctx.reply(`📋 نتيجة التنفيذ:\n\n${result}`);
      
      // اقتراح الخطوة التالية
      await ctx.reply('💡 هل هناك شيء آخر ترغب في القيام به؟ يمكنك طلب تحليل النظام، أو تنفيذ ميزة جديدة، أو إصلاح مشكلة.');
    } catch (error) {
