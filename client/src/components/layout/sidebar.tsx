  ...
  return (
    <div className="flex h-full flex-col border-l bg-white">
            ...
            return (
              <li key={item.name}>
                <Button
                  variant="ghost"
                  className={cn(
                    'h-10 w-full justify-start gap-3 px-3 text-sm font-normal',
                    location === item.href
      ...
      <div className="border-t border-gray-200 p-3">
        <Button
          variant="ghost"
          className="h-10 w-full text-sm font-normal text-black hover:bg-gray-50"
          onClick={() => logoutMutation.mutate()}
        >
          <LogOut className="ml-3 h-5 w-5" style={{ color: '#EA4335' }} />
          تسجيل خروج
        </Button>
      </div>
    </div>
