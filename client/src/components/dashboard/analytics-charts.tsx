...
export function SalesTrendsChart() {
  ...
  const [chartType, setChartType] = useState<"area" | "line" | "composed">(
    "area",
  );
...
export function CustomerGrowthChart() {
  ...
  const [chartView, setChartView] = useState<
    "growth" | "segments" | "retention" | "value"
  >("growth");
  ...
  const customerAnalytics = useMemo(() => {
    ...
    const segments = [
      ...
      {
        name: "عملاء نشطون",
        value: customers.filter((c) => sales.some((s) => s.customerId === c.id))
          .length,
      },
      {
        name: "عملاء متكررون",
        value: customers.filter(
          (c) => sales.filter((s) => s.customerId === c.id).length > 1,
        ).length,
      },
      {
        name: "عملاء مميزون",
        value: customers.filter(
          (c) =>
            sales
              ...
              .reduce((sum, s) => sum + Number(s.priceIqd) * s.quantity, 0) >
            1000000,
        ).length,
      },
    ];
    ...
    const retentionData = customers.reduce((acc: any[], customer) => {
      const monthYear = new Date(customer.createdAt).toLocaleDateString(
        "ar-IQ",
      ...
      .map((customer) => {
        ...
        const lastPurchase =
          customerSales.length > 0
            ? new Date(
                Math.max(
                  ...customerSales.map((s) => new Date(s.date).getTime()),
                ),
              )
        ...
        return {
          ...
          frequency,
          recency: Math.round(
            (Date.now() - lastPurchase.getTime()) / (24 * 60 * 60 * 1000),
          ),
        };
      })
    ...
    switch (chartView) {
      ...
      case "value":
        return (
          <RadarChart
            outerRadius={120}
            data={customerAnalytics.customerValue.slice(0, 5)}
          >
            <PolarGrid />
