        ...
        return (
          <ComposedChart data={chartData}>
            ...
            <Area
              ...
              stroke={colors.primary}
            />
            <Bar dataKey="count" name="عدد الطلبات" fill={colors.secondary} barSize={20} />
            <Line
              type="monotone"
              dataKey="avgOrder"
              name="متوسط الطلب"
              ...
              strokeWidth={2}
            />
            <Scatter dataKey="predictedAmount" name="التنبؤات" fill={colors.tertiary} />
          </ComposedChart>
        );
      ...
      <CardHeader>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <CardDescription>تحليل المبيعات اليومية والتنبؤات المستقبلية</CardDescription>
          </div>
    ...
    return products
      .map(product => {
        ...
        const totalRevenue = productSales.reduce(
          (sum, sale) => sum + Number(sale.priceIqd) * sale.quantity,
          0
        );
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={productPerformance}>
              ...
              <Tooltip />
              <Legend />
              <Bar yAxisId="left" dataKey="revenue" name="الإيرادات" fill={colors.primary} />
              <Bar
                yAxisId="right"
                dataKey="quantity"
                name="الكمية المباعة"
...
export function CustomerGrowthChart() {
  const [colorScheme, setColorScheme] = useState<ColorScheme>('default');
  const [updateInterval, setUpdateInterval] = useState(30000);
  const [chartView, setChartView] = useState<'growth' | 'segments' | 'retention' | 'value'>(
    'growth'
  );
  ...
  const customerAnalytics = useMemo(() => {
    ...
    const segments = [
      ...
      {
        name: 'عملاء نشطون',
        value: customers.filter(c => sales.some(s => s.customerId === c.id)).length,
      },
      {
        name: 'عملاء متكررون',
        value: customers.filter(c => sales.filter(s => s.customerId === c.id).length > 1).length,
      },
      {
        name: 'عملاء مميزون',
        value: customers.filter(
    ...
    const retentionData = customers.reduce((acc: any[], customer) => {
      const monthYear = new Date(customer.createdAt).toLocaleDateString('ar-IQ', {
        month: 'long',
        year: 'numeric',
      });
      ...
      if (existingMonth) {
        ...
        existingMonth.retained =
          (existingMonth.retained || 0) + (sales.some(s => s.customerId === customer.id) ? 1 : 0);
      } else {
        acc.push({
          month: monthYear,
          total: 1,
    ...
    // تحليل قيمة العميل
    const customerValue = customers
      .map(customer => {
        const customerSales = sales.filter(s => s.customerId === customer.id);
        const totalValue = customerSales.reduce(
          (sum, s) => sum + Number(s.priceIqd) * s.quantity,
          0
        );
            ...
            >
              {customerAnalytics.segments.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={
                    [colors.primary, colors.secondary, colors.tertiary, colors.accent][index % 4]
                  }
                />
              ))}
            </Pie>
        ...
        return (
          <ComposedChart data={customerAnalytics.growthData}>
            ...
            <Area
              ...
              fillOpacity={0.3}
            />
            <Bar dataKey="active" name="العملاء النشطون" fill={colors.secondary} barSize={20} />
            <Line
              type="monotone"
              dataKey="predicted"
      ...
      <CardHeader>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <CardDescription>تحليلات متقدمة لنمو وسلوك العملاء</CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Select
              value={colorScheme}
            ...
            <Tabs
              value={chartView}
              onValueChange={(value: typeof chartView) => setChartView(value)}
            >
              <TabsList>
